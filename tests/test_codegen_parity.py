...
from tinybird_sdk.api.resources import (
    DatasourceColumn,
    DatasourceEngine,
    DatasourceInfo,
    PipeInfo,
...
from tinybird_sdk.codegen.index import (
...
def test_type_mapper_and_utils_parity_behaviors() -> None:
    assert (
        clickhouse_type_to_validator("Nullable(LowCardinality(String))")
        == "t.string().low_cardinality().nullable()"
    )
    assert param_type_to_validator("Int32", 10, required=False) == "p.int32().optional(10)"
    assert to_camel_case("class") == "_class"
    assert "engine.replacing_merge_tree" in generate_engine_code(
...
def test_codegen_golden_fixtures() -> None:
    ...
    expected_datasources = (
        expected_base.joinpath("datasources.py").read_text(encoding="utf-8").rstrip() + "\n"
    )
    expected_pipes = expected_base.joinpath("pipes.py").read_text(encoding="utf-8").rstrip() + "\n"
    expected_client = (
        expected_base.joinpath("client.py").read_text(encoding="utf-8").rstrip() + "\n"
    )
