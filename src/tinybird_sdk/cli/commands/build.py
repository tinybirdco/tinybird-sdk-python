...
def run_build(options: BuildCommandOptions | dict[str, Any] | None = None) -> BuildCommandResult:
    ...
    normalized = (
        options
        if isinstance(options, BuildCommandOptions)
        else BuildCommandOptions(**(options or {}))
    )
    ...
    if normalized.dry_run:
        return BuildCommandResult(
            success=True, build=build_result, duration_ms=int(time.time() * 1000) - start
        )
    ...
    if dev_mode == "local":
        try:
            ...
            if config.get("is_main_branch") or not config.get("tinybird_branch"):
                ...
                workspace_name = get_local_workspace_name(
                    config.get("tinybird_branch"), config["cwd"]
                )
        ...
        if not normalized.token_override:
            try:
                ...
                dashboard_url = get_branch_dashboard_url(
                    config["base_url"], workspace.name, config["tinybird_branch"]
                )
